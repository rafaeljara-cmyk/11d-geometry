...
\begin{align}
\alpha_2^{-1}(M_Z) &= \sin^2\theta_W \cdot \alpha_{\text{EM}}^{-1}(M_Z) = 0.232 \times 128 = 29.7 \\
...
\begin{enumerate}
    ...
    \item \textbf{Proton decay at $\tau_p \approx 2.7 \times 10^{34}$ years.} The L-field GUT coupling predicts proton decay just above the current limit ($>2.4 \times 10^{34}$); the derivation is given in \cite{paper3}, Section~8. \textbf{Test:} Hyper-Kamiokande. If no decay at $\tau_p > 10^{35}$ years, the framework is falsified.
...
\midrule
Proton lifetime & $2.7 \times 10^{34}$ yr & $> 2.4 \times 10^{34}$ yr & \textbf{Consistent} \\
...
The proton decay prediction $\tau_p \approx 2.7 \times 10^{34}$ years is just above the current limit. Hyper-Kamiokande should detect proton decay; if no decay is seen at $\tau_p > 10^{35}$ years, the framework is falsified.
...
The framework makes 8 primary falsifiable predictions testable within the next decade. The most immediate: JUNO and DUNE will determine the neutrino mass hierarchy (predicted: normal), and proton decay searches at Hyper-Kamiokande will test the predicted lifetime $\tau_p \approx 2.7 \times 10^{34}$ years.
